...
- (void) oauthDidSuccess:(NSString *)accessToken accessSecret:(NSString *)accessSecret openid:(NSString *)openid openkey:(NSString *)openkey expireIn:(NSString *)expireIn {
    ...
    NSLog(@"%@-%@-%@-%@-%@",accessToken,accessSecret,openid,openkey,expireIn);
    //绑定信息写入plist文件中
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0];
    NSString *filePath = [documentsDirectory stringByAppendingPathComponent:@"tencentWBauthinfo.plist"];
    NSDictionary *new_userinfo = [[NSDictionary alloc] initWithObjectsAndKeys:accessToken,@"accessToken",openid,@"openid",openkey,@"openkey",nil];
    NSLog(@"%@",new_userinfo);
    [NSKeyedArchiver archiveRootObject:new_userinfo toFile:filePath];
    [OpenSdkBase showMessageBox:@"登录成功！"];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"notifytochangewbstate" object:nil userInfo:nil];
}
