				...
				success: function(data){
					...
					for(var i = 0; i < data.length; i++){
						...
						if(data[i].have_child == 1){
							src += "<img fieldid='"+data[i].ac_id+"' status='open' ectype='flex' src='templates/images/tv-expandable.gif' />";
						}else{
							src += "<img fieldid='"+data[i].ac_id+"' status='none' ectype='flex' src='templates/images/tv-item.gif' />";
						}
						//图片
						src += "</td><td class='w48 sort'>";
						//排序
						src += "<span title='可编辑' ajax_branch='article_class_sort' datatype='number' fieldid='"+data[i].ac_id+"' fieldname='ac_sort' ectype='inline_edit' class='editable'>"+data[i].ac_sort+"</span></td>";
						//名称
						src += "<td class='name'>";
						...
						if(data[i].have_child == 1){
							src += " <img fieldid='"+data[i].ac_id+"' status='open' ectype='flex' src='templates/images/tv-item1.gif' />";
						}else{
							src += " <img fieldid='"+data[i].ac_id+"' status='none' ectype='flex' src='templates/images/tv-expandable1.gif' />";
						}
						src += " <span title='可编辑' required='1' fieldid='"+data[i].ac_id+"' ajax_branch='article_class_name' fieldname='ac_name' ectype='inline_edit' class='editable'>"+data[i].ac_name+"</span>";
						//新增下级
						if(data[i].deep < 2){
