...
$db    = new DB();
	...
	$sql = "SELECT comment_id FROM  `".DB_PRE."comment_shop_app` WHERE `message` LIKE '%".$_GET['shop_name']."%'";
...
if ($_GET['shop_name'] != '') {
	$sql = "SELECT * FROM `".DB_PRE."comment_shop_app` WHERE `message` LIKE '%".$_GET['shop_name']."%' ORDER BY comment_id DESC limit {$offset},{$num}";
} else {
	$sql = "SELECT * FROM `".DB_PRE."comment_shop_app` ORDER BY comment_id DESC limit {$offset},{$num}";	
}
...
if ($_POST['form_submit'] == 'ok' && $_POST['submit_type'] == 'delete') {
	if (!empty($_POST['shop_id'])) {
		...
		$sql = "DELETE FROM `".DB_PRE."comment_shop_app` WHERE `comment_id` IN (".$str.")";
		$result = $db->query($sql);
		if ($result) {
...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        ...
        <li><a href="javascript:void(0);" class="current"><span>商家评论</span></a></li>
  ...
  <input name="page" value="1" type="hidden" />
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
          <th><label for="search_coupon_name">评论内容</label></th>
  ...
  <table class="table tb-type2" id="prompt">
    <tbody>
      ...
      <tr class="odd">
        <td>
        <ul>
            <li>通过搜索评论内容以快速找到评论信息，并进行相关操作。</li>
          </ul></td>
      </tr>
    </tbody>
  </table>
    <table class="table tb-type2">
      ...
      <?php foreach ($data_array as $val) {?>
		<tr class="hover edit">
          <td class="w24"><input name="shop_id[]" value="<?php echo $val['comment_id']?>" class="checkitem" type="checkbox"></td>
          <td class="name w300"><?php echo $val['message']?></td>
          <td class="align-center"><?php if ($val['mark'] == '3') { echo '推荐';} elseif ($val['mark'] == '2') { echo '一般';} elseif ($val['mark'] == '1') { echo '不推荐';}?></td>
          <td class="align-center"><?php echo $val['authorid']?></td>
