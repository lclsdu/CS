...
//	private $error_file = '';
	...
	public function upfile($field){
		...
		if(@copy($this->upload_file['tmp_name'],$this->save_path.DS.$this->file_name)){
			/*//产生缩略图
			if ($ifresize){
				require_once(BasePath.DS.'framework'.DS.'libraries'.DS.'resizeImage.php');
	...
	private function setPath(){
		...
		if (!is_dir($this->default_dir)){
			...
			$dir = strtr($dir, array('/'=>DS,'\\'=>DS));
			//$dir = str_replace('/', DS, $dir);
			$dir_array = explode(DS,$dir);
			$tmp_base_path = DISCUZ_DIR.DS.'data'.DS.'attachment'.DS.'forum';
			$num = count($dir_array);
			for($i=$num-2;$i<$num;$i++){
				$tmp_base_path = $tmp_base_path.DS.$dir_array[$i];
				if(!is_dir($tmp_base_path)){
					if (!@mkdir($tmp_base_path,0777)){
			...
			//新增一个index.html
			$fp = @fopen($this->default_dir.'/index.html','wb+');
			@fclose($fp);
		}
		
		//设置权限
		@chmod($this->default_dir,0777);

		/*//判断文件夹是否可写
		$test_file = $this->default_dir.DS.'index.htm';
	...
	private function setFileName(){
		$this->file_name = 'm'.md5(uniqid(rand(),true)).'.'.($this->new_ext==''?$this->ext:$this->new_ext);
	}
	 ...
	 * 记录未上传的文件信息
	 */
//	private function setErrorFileName($error_file){
//		$this->error_file .= "名为".$error_file."的文件存在错误，已被自动过滤  <br />";
//	}
	/**
	 * 根据系统设置返回商品图片保存路径
	 */
